"""Libya: the placement layer, Kontur 400 m population hexagons keyed to district.

Writes data/geo/ly/ly_hexes.gpkg.

Libya is almost all desert: Kufra, Murzuq and Ajdabiya are 900,000 km2 between them, half the
country, and hold 5% of its Libyans in the 2020 estimate. Spread flat, their dots would sit on the
sand seas; with Kontur an empty hex takes no dots.

THE JOIN IS ON HEX CENTROIDS, so a hex on a district line belongs to one side and nobody is counted
twice. Hexes whose centroid is outside every district (COD's coastline is coarse, and the `LY`
extract overruns the land borders) are snapped to the nearest district within `SNAP_KM` when they
hold people, and dropped beyond it.

THREE CHECKS AGAINST BSC'S 2020 ESTIMATE OF LIBYANS:

  * **the national ratio**, Kontur over the estimate, inside `EXPECTED_RATIO` +/- `TOLERANCE`.
    Kontur models everyone living in Libya and the estimate counts Libyans, so Kontur should read
    at or above 1; measured in a probe before this file was written at about 1.00;
  * **the rank witness for the join**: Kontur people per district against the estimate, Spearman,
    against every one of `N_PERM` shuffles of the estimate. A permuted join pairs Tripoli with Ghat
    and fails it;
  * **no town lost**: every district seat GeoNames lists with 50,000 or more people (PPLA, PPLC)
    is compared with Kontur's people within 5 and 10 km, and a seat under `HOLE_RATIO` counts as a
    hole only in a district whose Kontur total reads under `LOW_RATIO` of its estimate share
    (Morocco's gate, `sources/ma_grid.py`). The fixed 5 km test alone fails here on a correct
    grid: GeoNames gives Libya's seats their municipality populations (Tripoli 1,302,947, Al Khums
    201,943), and Kontur spreads Tripoli's district at about its average density, so Al Khums read
    0.096, Tripoli 0.105 and Benghazi 0.111 while every district's total sat at 0.98 to 1.06.
    `KONTUR_HOLES` names the holes, and none was found.

Usage:
    python sources/ly_grid.py --fetch    one gzipped gpkg from Kontur (4.4 MB); GeoNames LY.zip
    python sources/ly_grid.py            rebuild from data/raw/ly/
"""
...
GEO = os.path.join(ROOT, "data", "geo", "ly")
UNITS = os.path.join(GEO, "ly_districts.gpkg")
OUT = os.path.join(GEO, "ly_hexes.gpkg")

GZ_URL = ("https://geodata-eu-central-1-kontur-public.s3.amazonaws.com/kontur_datasets/"
          "kontur_population_LY_20231101.gpkg.gz")
GZ_NAME = "kontur_population_LY_20231101.gpkg.gz"
GPKG_NAME = "kontur_population_LY_20231101.gpkg"
GEONAMES_URL = "https://download.geonames.org/export/dump/LY.zip"
GEONAMES = os.path.join(RAW, "geonames_LY.zip")
...
N_PERM = 20_000
SNAP_KM = 2.0
SNAP_BANDS_KM = (0.5, 1.0, 2.0, 5.0, 10.0)   # printed, to show what the snap keeps

SEAT_MIN_POP = 50_000
HOLE_KM = 5.0
WIDE_KM = 10.0            # printed beside HOLE_KM
HOLE_RATIO = 0.10
LOW_RATIO = 0.5           # a district's Kontur share over its estimate share, below which seats count
KONTUR_HOLES = set()      # district seats Kontur has lost; asserted
...
def seat_check(out, units, names, rel):
    ...
    with zipfile.ZipFile(GEONAMES) as zf:
        t = pd.read_csv(zf.open("LY.txt"), sep="\t", header=None, names=GEONAMES_COLS,
    ...
    c["district"] = c["unit"].map(rel)
    c = c.sort_values("ratio")
    print(f"  Kontur within {HOLE_KM:g} and {WIDE_KM:g} km of each seat of {SEAT_MIN_POP:,}+, "
          f"lowest five of {len(c)}, with the district's own Kontur/estimate ratio:")
    for _i, r in c.head(5).iterrows():
        print(f"      {names[r['unit']]:<20} {r['seat']:<14} GeoNames {r['geonames']:>9,}   "
              f"Kontur {r['kontur']:>9,.0f} ({r['ratio']:.2f})  {r['kontur_wide']:>9,.0f} "
              f"({r['ratio_wide']:.2f})   district {r['district']:.2f}")
    holes = set(c.loc[(c["ratio"] < HOLE_RATIO) & (c["district"] < LOW_RATIO), "unit"])
    if holes != KONTUR_HOLES:
        raise SystemExit(f"seats with under {HOLE_RATIO:.0%} of their people in Kontur, in a "
                         f"district under {LOW_RATIO} of its share: {sorted(holes)}, not "
...
def main():
    ...
    if not os.path.exists(UNITS):
        raise SystemExit(f"missing {UNITS}; run sources/ly_geo.py first")

    hexes = read_layer(gpkg, "Kontur LY")
    ...
    if outside.any():
        ...
        print(f"\n  hexes whose centroid is outside every district: {int(outside.sum()):,} "
              f"({pts.loc[outside, popcol].sum():,.0f} people); people by distance to a district:")
    ...
    print(f"  dropped (beyond {SNAP_KM:g} km of every district): {int(dropped.sum()):,} hexes, "
    ...
    print(f"\n  Kontur {tot:,.0f} vs BSC 2020 Libyans {sum(est.values()):,}: ratio {ratio:.3f} "
          f"(expected about {EXPECTED_RATIO})")
    if abs(ratio - EXPECTED_RATIO) > TOLERANCE:
        raise SystemExit("Kontur and the 2020 estimate disagree beyond the band")

    # ---- the join witness: Kontur's rank of the districts against the estimate ----
    u = sorted(est)
    ...
    print(f"  join witness: Spearman(Kontur, estimate) over 22 districts = {rho:+.3f}; "
          f"{beaten} of {N_PERM:,} shuffles reach it (best {perm.max():+.3f})")
    if beaten:
        raise SystemExit("the rank witness fails; the district join may be permuted")

    print("\n  per-district Kontur 2023 / BSC 2020 Libyans, over the national ratio:")
    rel = {x: (per.loc[x, "sum"] / est[x]) / ratio for x in u}
    for x in sorted(rel, key=rel.get):
        print(f"      {names[x]:<20} {int(per.loc[x, 'size']):>6,} hexes   {rel[x]:5.2f}")
