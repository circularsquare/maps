"""Iraq — the placement layer: Kontur 400 m population hexagons, keyed to governorate.

Writes data/geo/iq/iq_hexes.gpkg.

**IRAQ IS MOSTLY EMPTY AND THE EMPTY PART IS HALF THE MAP.** Anbar, Muthanna and Najaf are
**217,674 km2 between them, 50.1% of Iraq's land, and 10.8% of its people**; the western
badiya and the southern desert have nobody in them, and the country's population is a strip
along the Tigris and the Euphrates plus the Kurdish north. Spread a governorate's dots evenly
over its polygon and Iraq draws as a solid block from the Syrian border to Basra, which is the
opposite of where anybody lives. Kontur removes that rather than patching it: an empty hex has
no population and takes no dots.

**AND IT MATTERS FOR WHAT THIS COUNTRY IS DRAWN FOR.** The whole content here is the Sunni and
Shia geography, and the two nearly meet along one line: Baghdad, the Diyala river valley,
Salah al-Din's Tigris towns, Kirkuk. Anbar's 2.0 million Sunnis live in Ramadi, Fallujah and
the Euphrates towns inside a few per cent of a polygon the size of England and Scotland
together; drawn flat, the difference between Anbar and Karbala would be painted mostly onto
gravel.
...
Hexes whose centroid falls outside every governorate are dropped and reported; Kontur's IQ
extract is cut to the country, so the number should be small.

Usage:
    python sources/iq_grid.py --fetch    one 8 MB gzipped gpkg from Kontur
    python sources/iq_grid.py            rebuild from data/raw/iq/
"""
...
GEO = os.path.join(ROOT, "data", "geo", "iq")
GOVERNORATES = os.path.join(GEO, "iq_governorates.gpkg")
OUT = os.path.join(GEO, "iq_hexes.gpkg")

GZ_URL = ("https://geodata-eu-central-1-kontur-public.s3.amazonaws.com/kontur_datasets/"
          "kontur_population_IQ_20231101.gpkg.gz")
GZ_NAME = "kontur_population_IQ_20231101.gpkg.gz"
GPKG_NAME = "kontur_population_IQ_20231101.gpkg"

EXPECTED_GOVERNORATES = 18
...
# denominator is the census enumerated 20-21 November 2024, so the grid should read LOW by
# about a year of Iraqi growth, which is roughly 2.5%. It is also worth knowing before the
# number is read that Iraq's pre-census projections ran about 43.3 million for 2023 against
# the count's 46.1 million, so a Kontur built on those inputs starts 6% short of the census
# for reasons that have nothing to do with the grid.
CENSUS_POPULATION = 46_118_793
...
def main():
    ...
    if not os.path.exists(GOVERNORATES):
        raise SystemExit(f"missing {GOVERNORATES} — run sources/iq_geo.py first")
    ...
    print("     Kontur's IQ extract is cut to the country and COD-AB's eighteen tile it, so "
          "this is\n     the seam between two renderings of the same border; dropped.")
    ...
    # whole polygon — which in Iraq means over the badiya.
    ...
    print(f"\n  Kontur {tot:,.0f} vs the 2024 census {CENSUS_POPULATION:,} — ratio "
          f"{ratio:.3f}")
    ...
    print("\n  per-governorate Kontur/census ratio (the shape check):")
    census = dict(zip(govs["unit"], govs["pop"]))
    names = dict(zip(govs["unit"], govs["name"]))
    rows = [(names[u], u, int(r["size"]), r["sum"] / census[u]) for u, r in per.iterrows()]
