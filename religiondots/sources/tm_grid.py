"""Turkmenistan — the placement layer: Kontur 400 m population hexagons, keyed to velayat.

Writes data/geo/tm/tm_hexes.gpkg.

Six units over 491,200 km2 (the census's own figure), and most of it is the Karakum. Balkan is
150,000 km2 with its people on the Caspian coast, along the railway and at Balkanabat; Lebap
and Dashoguz live along the Amu Darya; Mary and Ahal in the Murgab and Tejen oases and the
Kopetdag foothills. Spread any velayat's dots over its polygon and most of the colour lands on
sand.

THE JOIN IS SPATIAL, on hex CENTROIDS, so a hex on a velayat line belongs wholly to one side.
Hexes whose centroid falls outside every unit are dropped and reported. This is
`sources/uz_grid.py` with the paths changed.

Usage:
    python sources/tm_grid.py --fetch    one ~2.6 MB gzipped gpkg from Kontur
    python sources/tm_grid.py            rebuild
"""
...
GEO = os.path.join(ROOT, "data", "geo", "tm")
REGIONS = os.path.join(GEO, "tm_regions.gpkg")
LOOKUP = os.path.join(GEO, "tm_lookup.csv")
OUT = os.path.join(GEO, "tm_hexes.gpkg")

GZ_URL = ("https://geodata-eu-central-1-kontur-public.s3.amazonaws.com/kontur_datasets/"
          "kontur_population_TM_20231101.gpkg.gz")
GZ_NAME = "kontur_population_TM_20231101.gpkg.gz"
GPKG_NAME = "kontur_population_TM_20231101.gpkg"

EXPECTED_UNITS = 6

# Kontur is modelled (GHSL, HRSL, building footprints), not a census, and is used only as a
# within-velayat weight. Its vintage is 2023-11 against the census's 17 December 2022.
OFFICE_POPULATION = 7_057_841
...
def main():
    ...
    if not os.path.exists(REGIONS):
        raise SystemExit(f"missing {REGIONS} — run sources/tm_geo.py first")
    ...
    print(f"\n  Kontur {tot:,.0f} vs the census's {OFFICE_POPULATION:,} — ratio {ratio:.3f}")
